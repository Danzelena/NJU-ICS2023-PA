...
// *dummy.c         +++load-store.c  +++mul-longlong.c  *shuixianhua.c  *wanshu.c
  ...
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J, R(rd) = s->pc + 4; s->dnpc = s->pc + imm;ftrace_call(s->dnpc));
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, R(rd) = s->pc + 4;s->dnpc = ((imm + src1)&~1);ftrace_call(s->dnpc));
  ...
  // s9      :aeb1c2aa
  // a5      :aeb1c2aa
  INSTPAT("0000001 ????? ????? 001 ????? 01100 11", mulh   ,R2, R(rd) = (sword_t)SEXT((int64_t)((int32_t)src1 * (int32_t)src2) , XLEN));
