...
// *dummy.c         +++load-store.c  +++mul-longlong.c  *shuixianhua.c  *wanshu.c
  ...
  // mret
  // pc->CSRs[mepc],.......
  INSTPAT("0011000 00010 00000 000 00000 11100 11", mret   , J, s->dnpc = cpu.INTR ? SR(mepc): SR(mepc) + 4; if(cpu.INTR){SR(mstatus) = set_bit(SR(mstatus), MIE_OFF, MPIE(SR(mstatus))); SR(mstatus) = set_bit(SR(mstatus), MPIE_OFF, 1);});
