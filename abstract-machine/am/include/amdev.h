...
AM_DEVREG(11, GPU_FBDRAW,   WR, int x, y; const void *pixels; int w, h; bool sync);// 帧缓冲控制器, 向(x,y)处绘制w * h的矩形图像, 图像像素按行优先方式存储在pixels中, 每个像素用32位整数以00RRGGBB的方式描述颜色. 若sync为true, 则马上将帧缓冲中的内容同步到屏幕上.
